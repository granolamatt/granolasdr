...
static const WF_ELEM_T* get_cand_mag(const ftx_waterfall_t* wf, const ftx_candidate_t* candidate)
{
    ...
    // printf("Offset for %d is %d timesub %d freqsub %d\n", candidate->time_offset, offset, candidate->time_sub, candidate->freq_sub);
...
// Search a slice of the frequency axis [freq_offset_start, freq_offset_end).
// Pass freq_offset_end = -1 to search to the end of the band.
// Returns heap_size WITHOUT sorting — caller must sort after merging slices.
int ftx_find_candidates_range(const ftx_waterfall_t* wf, int num_candidates,
                               ftx_candidate_t heap[], int min_score,
                               int freq_offset_start, int freq_offset_end)
{
    ...
    int freq_end = (freq_offset_end < 0) ? wf->num_bins : freq_offset_end;
    ...
    for (candidate.time_sub = 0; candidate.time_sub < wf->time_osr; ++candidate.time_sub)
    {
        ...
        {
            for (candidate.time_offset = 0; candidate.time_offset < 30; ++candidate.time_offset)
            {
                for (candidate.freq_offset = freq_offset_start;
                     candidate.freq_offset < freq_end && (candidate.freq_offset + num_tones - 1) < wf->num_bins;
...
{
    int heap_size = ftx_find_candidates_range(wf, num_candidates, heap, min_score, 0, -1);

    // Sort descending by score (heapsort over the min-heap)
