...
typedef struct
{
    ...
    int32_t time_offset; ///< Index of the time block
    int32_t freq_offset; ///< Index of the frequency bin
...
/// Like ftx_find_candidates but searches only freq_offset in [freq_offset_start, freq_offset_end).
/// Pass freq_offset_end < 0 to search through wf->num_bins. Returns heap unsorted — caller must sort after merging.
