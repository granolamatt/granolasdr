...
def extract_logl_db(A2db):
    ...
    print("Shape is", A2db_bits.shape)
    hold = A2db_bits
    a = [
        A2db[7, :] - A2db[0, :],
        A2db[3, :] - A2db[0, :],
...
for f0, time_start in candidates:
    ...
    vv = np.array([[0,0,1000,0,0,0,0,0]]).T

    extract_logl_db(vv)
    exit()

    bits_logl, A2db_bits = extract_logl_db(A2db)
    (num_errors, bits, iter) = ldpc.bp_solve(bits_logl, max_iters=30, max_no_improvement=15)
