    ...
    int num_bins, int num_blocks, int time_osr, int freq_osr, int min_score)
{
    ...
    // Compute Costas sync score entirely from shared memory.
    ...
    for (int m = 0; m < 3; ++m) {
        for (int k = 0; k < 7; ++k) {
            ...
            int sm  = kCostas[k];
            int si  = (m * 7 + k) * WINDOW + threadIdx.x;  // base index in smem for this slot+thread
