#ifndef _GM_CUDA_FT8LDPCCUDA_H_
#define _GM_CUDA_FT8LDPCCUDA_H_

#include <cuda_runtime.h>
#include <cstdint>
#include "gm/cuda/FT8ScanCuda.h"

// FT8 LDPC star-sum SP flooded decoder — GPU stream-aware launcher.
//
// Call ft8_ldpc_init_constants() once (in FT8Cuda constructor) to copy the
// FT8 (174,83) H-matrix into __constant__ memory.
//
// ft8_sp_flooded_decode_batch: exact log-domain sum-product with Gallager phi,
// flooding schedule, 50 iterations; ~3.0 KB shared memory per block.

#define FT8_LDPC_BATCH FT8_GPU_CAND_MAX

void ft8_ldpc_init_constants();

void ft8_sp_flooded_decode_batch(
    uint32_t     n_candidates,
    const float* log174_d,
    uint8_t*     x_hat_d,
    bool*        parity_d,
    cudaStream_t ldpc_stream);

#endif // _GM_CUDA_FT8LDPCCUDA_H_
