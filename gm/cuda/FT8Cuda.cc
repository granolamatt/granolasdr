        ...
        if (buff_pos > 2*rfft_length) {
            ...
            if (cont_scan_active.load(std::memory_order_acquire) &&
                ...
                if (wi - ri >= (uint64_t)CONTINUOUS_SLOTS) {
                    fprintf(stderr, "cont_slots full, dropping at ring_write_idx=%lu\n",
                    ...
                    cudaMemcpyAsync(slot.log174, slot.log174_d,
                                    (size_t)CONT_CAND_MAX * kFtxLdpcN * sizeof(float),
                                    cudaMemcpyDeviceToHost, cont_scan_stream);
...
void FT8Cuda::allocContSlots() {
    const size_t log174_bytes = (size_t)CONT_CAND_MAX * kFtxLdpcN * sizeof(float);
    ...
    printf("Continuous scan: %d slots × %.1f MB = %.1f MB device + %.1f MB pinned\n",
           CONTINUOUS_SLOTS,
           (double)(log174_bytes + CONT_CAND_MAX * (4+1+1+1+2)) / 1e6,
           (double)CONTINUOUS_SLOTS * (log174_bytes + CONT_CAND_MAX * (4+1+1+1+2)) / 1e6,
           (double)CONTINUOUS_SLOTS * (log174_bytes + CONT_CAND_MAX * (4+1+1+1+2)) / 1e6);
}
...
void FT8Cuda::contWorker() {
    ...
    while (cont_scan_active.load(std::memory_order_acquire)) {
        ...
        // Wait for dispatched flag (ensures timestamp is visible too).
        ...
        if (n > 0 && decode_callback) {
            n = std::min(n, CONT_CAND_MAX);
            *slot.count = n;
            printf("[CONT] ring=%lu cands=%u\n", (unsigned long)(ri), n);
            try {
                decode_callback(slot);
            } catch (...) {
                fprintf(stderr, "[CONT] decode_callback threw exception at slot %lu\n",
