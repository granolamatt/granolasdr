...
namespace gm {
namespace cuda {
FreqView::FreqView(gm::buffer::BufferPosition<std::complex<short>>* inP) :
    inPos(inP),
    ...
    outPos(), epoch(0)
{
        ...
        std::complex<float>* outData = (std::complex<float>*)calloc(NLARGE * NSBUFFERS, sizeof(std::complex<float>));
...
long FreqView::capture() {
    try
    {
        ...
        cudaMemcpy(&outPos.getBuffer()[bufferNum*NLARGE], fft_d, 2*length, cudaMemcpyDeviceToHost);
